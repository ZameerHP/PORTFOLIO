...
# now replace the inline map
inline_map_regex = re.compile(r'\{PROJECTS\.map\(\(project, idx\) => \{.*?</motion\.div>\s*\)\s*\}\s*\)', re.DOTALL)

# Let's verify what the inline map actually ends with
