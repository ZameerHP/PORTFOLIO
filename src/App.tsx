...
export default function App() {
  ...
  const slideCount = 9; // 0 to 8 sections
  ...
  const changeSlide = (direction: "next" | "prev" | number) => {
    ...
    if (typeof direction === "number") {
      ...
      targetIndex = Math.min(sectionsCount - 1, currentSlide + 1);
    } else {
  ...
  // Set up scroll tracking and IntersectionObserver
  useEffect(() => {
    if (isLoading) return;
    const container = containerRef.current;
    if (!container) return;

    // Track scroll to update splitProgress for WORK text split
    const handleScroll = () => {
      const scrollY = container.scrollTop;
      const vh = window.innerHeight || 800;
      const startScroll = 3 * vh;
      const endScroll = 6 * vh;

      if (scrollY < startScroll) {
        splitProgress.set(0);
      } else if (scrollY > endScroll) {
        splitProgress.set(300);
      } else {
        const progress = ((scrollY - startScroll) / (endScroll - startScroll)) * 300;
        ...
        if (entry.isIntersecting) {
          ...
          setCurrentSlide((prevActive) => {
            if (prevActive !== index) {
              ...
              setSlideDirection(index > prevActive ? "next" : "prev");
            }
            return index;
          });
        }
    ...
    >
      ...
      <header className="fixed top-10 left-6 md:left-[60px] right-6 md:right-[60px] z-40 flex items-center justify-between pointer-events-none select-none" id="nav-header">
        ...
        <div 
          ...
          className={`cursor-pointer hover:opacity-80 transition-opacity pointer-events-auto ${(currentSlide === 1 || (currentSlide >= 3 && currentSlide <= 5)) ? "text-black" : "text-white"} ${currentSlide === 2 ? "opacity-0 pointer-events-none" : "opacity-100"}`}
          id="nav-logo"
        >
          {currentSlide >= 3 && currentSlide <= 5 ? (
        ...
        <div className="flex items-center gap-[12px] pointer-events-auto" id="nav-right-group">
          {/* Mobile Menu Toggle */}
          <button 
            className={`lg:hidden font-mono text-[11px] font-bold uppercase tracking-widest px-[16px] h-[44px] md:h-[44px] flex items-center justify-center rounded-full transition-all duration-300 ${currentSlide === 1 || currentSlide === 2 || (currentSlide >= 3 && currentSlide <= 5) ? "text-black border border-black/20" : "text-white border border-white/20"}`}
          ...
          <div className="hidden lg:flex items-center gap-[8px]" id="nav-links">
            {[
              ...
              const isDarkText = currentSlide === 1 || currentSlide === 2 || (currentSlide >= 3 && currentSlide <= 5);
              const isActive = item.name === "Works" 
                ? (currentSlide >= 3 && currentSlide <= 5) 
          ...
          <button
            ...
            onMouseEnter={() => setCursorType("hover")}
            onMouseLeave={() => setCursorType("default")}
            className={`w-[185px] h-[44px] font-sans rounded-full flex items-center justify-between pl-4 pr-1 transition-all duration-700 shadow-xl shadow-black/20 group ${currentSlide === 1 || currentSlide === 2 || (currentSlide >= 3 && currentSlide <= 5) ? "bg-black text-white hover:bg-neutral-800" : "bg-white text-[#111111] hover:bg-neutral-100"}`}
            id="nav-cta-btn"
          >
      ...
      {/* OUTER SNAP CONTAINER */}
      <div 
        ref={containerRef}
        id="outer-snap-container" 
      ...
      >
        ...
        <section 
          data-index={0} 
          id="section-hero"
          className="w-full h-full snap-start relative overflow-hidden bg-[#050505] flex flex-col justify-between"
        >
        ...
        <section 
          data-index={1} 
          id="section-about"
          className="w-full h-full snap-start relative overflow-hidden bg-[#ff4f1d]"
        >
          <motion.div
            initial="initial"
            whileInView="animate"
        ...
        <section 
          data-index={2} 
          id="section-services"
          className="w-full h-full snap-start relative overflow-hidden bg-[#F6EEDC]"
        >
          <div className="w-full h-full overflow-y-auto custom-scrollbar">
            ...
            <div className="w-full max-w-[1600px] mx-auto px-6 md:px-[120px] py-[100px] md:py-[120px]">
              {/* Header */}
              <motion.div
                initial={{ opacity: 0, y: 40 }}
        ...
        <section 
          data-index={3} 
          id="section-work-0"
          className="w-full h-full snap-start relative overflow-hidden bg-[#F6EEDC] flex items-center justify-center"
        >
          <ProjectCard project={PROJECTS[0]} idx={0} />
        </section>
        ...
        <section 
          data-index={6} 
          id="section-skills"
          className="w-full h-full snap-start relative overflow-hidden bg-[#0d0d0d]"
        >
        ...
        <section 
          ...
          className="w-full h-full snap-start relative overflow-hidden bg-[#121212] flex flex-col justify-center"
        >
          <motion.div 
            initial="initial"
            whileInView="animate"
            viewport={{ once: false, amount: 0.3 }}
            className="max-w-7xl mx-auto px-6 md:px-12 w-full space-y-8" 
        ...
        <section 
          data-index={8} 
          id="section-faq"
          className="w-full h-full snap-start relative overflow-hidden bg-[#121212]"
        >
          <div className="w-full h-full overflow-y-auto pt-24 pb-12 custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8" id="footer-grid">
                  ...
                  <div className="md:col-span-3 space-y-3" id="footer-links">
                    ...
                    <div className="flex flex-col gap-1.5 text-xs font-mono tracking-widest uppercase text-white/50" id="footer-explore-links">
                      ...
                      <button onClick={() => changeSlide(6)} className="hover:text-white text-left transition-colors py-2 block">Skills</button>
                      <button onClick={() => changeSlide(7)} className="hover:text-white text-left transition-colors py-2 block">Reviews</button>
                      <button onClick={() => changeSlide(8)} className="hover:text-white text-left transition-colors py-2 block">Contact</button>
                    </div>
                  </div>
